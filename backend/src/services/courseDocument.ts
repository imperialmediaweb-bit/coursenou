...
<script>
  // These have to live in a script block rather than as onclick/onerror
  // attributes: the app's Content-Security-Policy sets script-src-attr 'none',
  // which blocks every inline handler, so the button silently did nothing.
  document.getElementById('print-button').addEventListener('click', function () {
    window.print();
  });

  // Drop the frame entirely when a lesson image fails to load, so the export
  // has no empty grey boxes in it.
  document.querySelectorAll('img[data-drop-on-error]').forEach(function (img) {
